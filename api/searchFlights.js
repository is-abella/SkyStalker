...
async function findCheapestFlights(flightInfo) {
	...
	if (way === "Return") {
		...
		while (arrivalDateElement === "") {
			try {
				arrivalDateElement = await driver.wait(
					until.elementLocated(By.xpath(returnDateXpath)),
					5000
				);
				await driver.sleep(1000);
			...
			for (let i = 0; i < counter; i++) {
				if (flightWay === "Return") {
					const goThere = {
						...
						flightTiming: flights[i].querySelectorAll(".vmXl.vmXl-mod-variant-large")[0]
							.innerText,
						flightDuration: flights[i].querySelectorAll(
							".vmXl.vmXl-mod-variant-default"
						)[1].innerText,
					};
					const comeBack = {
						stops: flights[i].querySelectorAll(".JWEO-stops-text")[1].innerText,
						flightTiming: flights[i].querySelectorAll(".vmXl.vmXl-mod-variant-large")[1]
							.innerText,
						flightDuration: flights[i].querySelectorAll(
							".vmXl.vmXl-mod-variant-default"
						)[3].innerText,
